...
// Compute current streak: consecutive days ending today (or yesterday) with any log entry or rest day
function computeStreak(workoutDates, foodDates, ratingDates, restDates) {
  const allActiveDates = new Set([...workoutDates, ...foodDates, ...ratingDates, ...(restDates || [])])
...
export default function ProgressTab() {
  ...
  const streak = computeStreak(workoutDates, foodDates, ratingDates, restDays)
  ...
  // Build weight chart data (last 30 days), converted to display unit
  const weightChartData = days.map(date => {
    const entry = weightLog.find(w => w.date === date)
    if (!entry) return null
    const displayVal = weightUnit === 'lbs'
      ? Math.round(entry.kg * 2.205 * 10) / 10
      : entry.kg
    return { date: shortDate(date), weight: displayVal }
  }).filter(Boolean)
    ...
    <div className="p-4 space-y-4 pb-6">
      ...
      <div className="bg-slate-800 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {weightChartData.length >= 2 && (() => {
              const delta = (weightChartData[weightChartData.length - 1].weight - weightChartData[0].weight).toFixed(1)
              ...
              return (
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isDown ? 'text-emerald-400 bg-emerald-900/40' : isUp ? 'text-red-400 bg-red-900/40' : 'text-slate-400 bg-slate-700'}`}>
                  {isDown ? '▼' : isUp ? '▲' : '—'} {Math.abs(delta)} {weightUnit} (30d)
                </span>
              )
            })()}
            <div className="flex gap-1">
              {['kg', 'lbs'].map(u => (
                <button
                  ...
                  className={`text-xs px-2 py-0.5 rounded-full font-medium transition-colors ${weightUnit === u ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-400'}`}
                >
                  {u}
                </button>
              ))}
        ...
        {weightChartData.length > 0 && (
          <div className="mb-3 text-center">
            ...
            <span className="text-slate-400 text-sm ml-1">{weightUnit}</span>
          </div>
        )}
          ...
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={140}>
              <LineChart data={weightChartData} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
                ...
                <YAxis tick={tickStyle} domain={['auto', 'auto']} unit={weightUnit === 'lbs' ? ' lb' : ' kg'} />
                <Tooltip contentStyle={tooltipStyle} formatter={v => [`${v} ${weightUnit}`, 'Weight']} />
                <Line
                  type="monotone"
                  dataKey="weight"
                  stroke="#a78bfa"
      ...
      <div className="bg-slate-800 rounded-xl p-4">
        ...
        <ResponsiveContainer width="100%" height={150}>
          <LineChart data={chartData} margin={{ top: 4, right: 48, left: -10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 2"
              strokeOpacity={0.5}
              label={{ value: `Goal ${targets.calories}`, position: 'right', fill: '#f97316', fontSize: 10 }}
            />
            <Line
              type="monotone"
              dataKey="calories"
      ...
      {exerciseNames.length > 0 && (
        <div className="bg-slate-800 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-slate-300">Exercise Progress</h3>
          </div>
          {(exerciseProgression.length < 2) && (
            <p className="text-xs text-slate-500 mb-3">Tap an exercise to see your strength over time</p>
          )}
          {exerciseProgression.length >= 2 && (
            <p className="text-xs text-slate-400 font-medium mb-3">{selectedExercise}</p>
          )}
          <select
